...
> **Trzy filary przez cały kurs:** ograniczone zaufanie · grounding (zakotwiczenie w danych) · `privacy cage` (klatka prywatności).

::: notes
Podkreślam praktyczność: nie wychodzimy z bloku bez artefaktu — szablonu promptu, mapy ryzyka, matrycy literatury, karty privacy cage. Wszystkie szablony są w folderze kurs-pe, więc nikt nie zaczyna od pustej kartki. I jeszcze jedno: nie polecam Notion na materiały badawcze — trzyma treść na serwerze, co łamie zasadę klatki prywatności. DEVONthink pokażę tylko jako mój przykład zaawansowanej bazy wiedzy — Państwo go nie potrzebują.
:::
...
::: notes
To jest serce kursu. Jeśli zapamiętają Państwo tylko jeden slajd — ten. Ograniczone zaufanie: model generuje statystycznie prawdopodobny ciąg, więc potrafi zmyślić cytat brzmiący jak prawdziwy. Grounding: zawsze pytam „na czym to opierasz?" i żądam cytatu z podanego materiału. Privacy cage: zanim cokolwiek wkleję, pytam „gdzie trafią te dane?". Cała reszta to techniki realizujące te trzy zasady.
:::
...
| **Grounding** | Zakotwiczenie odpowiedzi w materiale i źródłach. |
...
::: notes
Nie omawiam tej tabeli punkt po punkcie — wróci do niej, gdy każde pojęcie pojawi się w praktyce. Teraz tylko sygnalizuję, że każdy termin techniczny wyjaśnię jednym prostym zdaniem, zanim go użyję. Żargonu informatycznego unikamy.
:::
...
**`privacy cage` — klasyfikacja:** jawne / wrażliwe / osobowe / medyczne / pod NDA → decyzja, co wolno wkleić.
...
*(DEVONthink — tylko przykład zaawansowanego RAG prowadzącego; Notion — odpada, serwer = poza `privacy cage`.)*
...
**Czym jest:** narzędzie, które **wczytuje wiele źródeł** (PDF, strony, wideo, własne notatki) i pozwala je odpytywać z **groundingiem w tych źródłach**.
...
- **chmura Google → uwaga `privacy cage`** (nie wgrywaj danych wrażliwych),
...
# DZIEŃ 3 — Agenci AI i privacy cage {background-color="#1a2b4a"}
...
> *„Głęboki research, płytka sprawczość"* — narzędzia świetnie **zbierają**, ale **nie zastępują oceny badacza**.
...
- **Po co lokalnie:** dane **nie opuszczają komputera** — rdzeń `privacy cage`.
- **Ograniczenia:** słabsze od modeli frontier · brak wiedzy webowej · wyższe ryzyko halucynacji · wolniej.
...
::: notes
To jest moment, gdy privacy cage przestaje być hasłem, a staje się konfiguracją. Są dane, których po prostu nie wolno wysłać do chmury — i wtedy lokalny model bywa jedyną drogą do użycia AI, za cenę jakości. Uczciwie mówię o kompromisie: „działa lokalnie" czasem znaczy „czeka się na każde zdanie". Każdy — niezależnie od sprzętu — wypełnia kartę decyzyjną privacy cage: typ danych mapujemy na środowisko frontier / lokalny / bez AI.
:::
...
**Nie każdy model nadaje się do *tool calling* i RAG.**
...
> 🇵🇱 **Bielik / Bielik-Minitron** — doskonałe rozumienie polszczyzny, świetne **chatboty** — ale to **NIE są** modele agentyczne. Do klastrowania czy tool callingu wybierz model agentyczny.
...
## B10 · Kwantyzacja — jak wybrać wariant {.smaller}
...
> **Człowiek w pętli** + praca na kopii folderu + reguły `privacy cage` zaszyte w instrukcji.
...
::: notes
To zwieńczenie całego kursu i dowód, że agent to nie magia, lecz prompt plus kontekst plus pliki instrukcji plus nadzór człowieka — wszystko, co budowaliśmy przez trzy dni. Demonstrację robię na żywo, na moim komputerze, więc na slajdzie zostają tylko ramy; szczegóły i pełny pipeline są w katalogu clustering-demo i w pliku-wskaźniku modul-clustering-demo. Na pokazie podłożę do folderu summaries znacznie więcej plików — pięćdziesiąt do stu — żeby było widać prawdziwą skalę możliwości modeli agentycznych. Podkreślam dwie rzeczy: po pierwsze, to samo odpalę OpenCode z modelem lokalnym, więc streszczenia nie opuszczają komputera — privacy cage w praktyce; po drugie, outputem jest nie tylko raport, ale i OPML — czyli struktura, którą znamy z dnia drugiego, gotowa do napisania własnego przeglądu literatury.
:::
...
**Zaliczenie:** min. „częściowo" w każdym kryterium + kompletny `PROTOKOL-AI.md`.
...
**Łuk kursu:** zapytania → kontekst → agenci. **Trzy filary:** ograniczone zaufanie · grounding · `privacy cage`.
